...
if not conf.randomPrimers and len(sys.argv) != 4:
    ...
    sys.stdout.write("** Random primers mode **\n\nUsage: python RTEventsCounter.py samFilePath.sam refFilePath.fa\n")
    sys.exit()
...
try:
    ...
    def parseSamLine(line):
        ...
        # Get NM (edit dist) and MD (mismatches) optional fields, if any
        # NMfield = MDfield = None
        # if len(splitLine) > 11:
        #     for tag in splitLine[11:]:
        #         if NMfield == None and tag[:5] == "NM:i:":
        #             NMfield = int(tag[5:])
        #         if MDfield == None and tag[:5] == "MD:Z:":
        #             MDfield = tag[5:]
        #         if NMfield != None and MDfield != None:
        #             NMfield = MDfield = None
        #             break
    ...
    # Note down if random primer mode is on
        ...
        if clusterName == "skip":
            ...
            if conf.alignPaired:
                if not pairFound:
                    ...
                    pairFound = False
                    updateCounts = False
            elif prevClusterName != "" and not conf.ignoreUnpaired:
                updateCounts = True
                eventsToWrite = dict(prevEvents)
                qualsToWrite = dict(prevQualities)
    ...
    # Prepare printing nt number shift if 1-based; placed here to avoid looped lookup
    if conf.ntBase1:
        baseShift = 1
    else:
        baseShift = 0
    ...
    for seqName in refSeqs:
        ...
        for primerName in primerKeyList[seqName]:
            ...
            for i in printRange:
                ...
                # Build Pstop
                # Use read-through of 3 prime neighboring nuc when available
                if i+1 in printRange:
                    Pstop = safeDivide(stopCount, RT[seqName][primerName][i+1])
                else:
                    Pstop = safeDivide(stopCount, nucRT)
